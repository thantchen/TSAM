...
<template>
    <div id="escalation-log">
        <div class="vx-row" style="margin-top: 10px">
            <div class="vx-col w-full">
                <vx-card title="Performance Standard Escalation">
                    <div class="add_button"><vs-button color="primary" type="border" size="small" style="margin-bottom: 10px; margin-left: -10px" icon="add_box" @click="openRequestLogDialog">Submit Performance Standard Escalation Request</vs-button></div>     
                    ...
                    <ejs-grid ref='grid' :dataSource="data" :allowSorting='true' :allowFiltering='true' :allowResizing='true' :filterSettings='filterSettings' :allowPaging='true' :pageSettings='pageSettings'>
                        <e-columns>
                            <e-column field='changeLogId' headerText='CHANGE ID' isPrimaryKey='true' :template='commandTemplate'></e-column>
        ...
        <vs-prompt id="dialog" :active.sync="isDialogVisible" :title="dialogTitle" :is-valid="validate" @accept="submitRequest" @close="onCloseRequest" @cancel="cancelRequest" accept-text="Submit" :cancel-text="cancelText" >
            <div id="dialog-container" class="vs-con-loading__container" >
                ...
                <vs-row>
                    ...
                    <vs-col vs-type="flex" vs-w="4" v-if="logData.changeId != 0">
                        <div class="vs-component" style="width: 100%;">
                            <label for="changeId" class="vs-input--label">Change ID</label>
                            <vs-input v-model="logData.changeId" id="changeId" readonly style="width:100%" :disabled="!isNewSubmission"/>
                        </div>
                    </vs-col>                                     
                </vs-row>
...
export default{
    ...
    data: function() {
        return {
            ...
            uploadAttachmentsPath: {
                saveUrl: `${process.env.VUE_APP_SERVICE_API_URL}/api/upload/change-log-attachments`,
            },
            cancelText: 'Cancel',
            ...
            commandTemplate: function () {
                return {
                    template: Vue.component('anchorTemplate', {
                        template:  '<a href="#" @click="onClick()">{{data.changeId}}</a>',
                        data() { return { data: { data: {} } }; },
                        methods: {
                            onClick: function() {
                                this.$eventHub.$emit('openRequestLogDialog', this.data.changeLogId);
                            }
                        }          
                    })
                }
    ...
    methods:{
        ...
        async submitRequest() {
            ...
            try {
                ...
                let response = await this.$http.put(`${process.env.VUE_APP_SERVICE_API_URL}/api/input/change-log`, formData);
                response.data.data.forEach((item) => {
                    var index = this.data.findIndex(log => log.changeLogId == item.changeLogId);
                ...
                this.logData.changeLogIds = response.data.data.map(m => m.changeLogId);
                ...
                if (isNewRecord && attachmentCount == 0) {  // If attachmentCount > 0, we'll send email after all attachment(s) finish uploading and then send email
                    // Call Add New API
                    this.sendEmail("newChangeRequest", this.logData.changeLogIds);
                }
                else if (isNewRecord == false ) {
                    // Call Add Update API
                    this.sendEmail("updatedChangeRequest", this.logData.changeLogIds);
                }
        ...
        async loadLogData(log) {
            ...
            this.logData.changeLogIds = log.changeLogIds;
            this.logData.attachments = log.attachments;

            if (this.logData.attachments == undefined) {
                this.logData.attachments = await this.httpGet(`/api/input/change-log/${this.logData.changeLogId}/attachments`);
                
                var log1 = this.findLog(this.logData.changeLogId);
        ...
        async loadLogs() {
            this.data = await this.httpGet(`/api/input/escalation-log`);
        },
        async getLookupData(apiEndpoint) {
            return this.httpGet(`${apiEndpoint}`);
        },
        findLog(logId) {
            return this.data.find(item => item.changeLogId == logId)
        },
        ...
        onUploadSuccess: function(args) {
            ...
            if (file) {
                this.logData.changeLogIds.forEach(function(logId) {
        ...
        async onUploadActionComplete(args) {
            ...
            if (failed.length > 0) {
                ...
                await this.sendEmail("newChangeRequest", this.logData.changeLogIds);
        ...
        onUploading: function(args) {
            ...
            args.customFormData = [ { 'logIds': this.logData.changeLogIds } ];
        },
    ...
    computed: {
        dialogTitle: function() {
            return "Performance Standard Escalation Request" + (this.isNewSubmission ? " Form" : " - " + this.selectedLogType);
        },
        validate: function() {
    ...
    async mounted() {
        ...
        const logTypes = await this.getLookupData("/api/lookup/log-types");
        this.logTypeOptions = logTypes.filter(function(item) {
            return item.name.toLowerCase() == "escalation";
        })
